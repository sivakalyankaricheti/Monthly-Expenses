...
const privacyHtml = await readFile(new URL('./public/privacy.html', import.meta.url));
...
async function initDb(){
  ...
  CREATE TABLE IF NOT EXISTS tracker_state_history(id bigserial PRIMARY KEY,user_id text NOT NULL REFERENCES users(user_id) ON DELETE CASCADE,data jsonb NOT NULL,created_at timestamptz NOT NULL DEFAULT now());
  CREATE INDEX IF NOT EXISTS tracker_state_history_user_created_idx ON tracker_state_history(user_id,created_at DESC);
  ...
  await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS phone text; ALTER TABLE users ADD COLUMN IF NOT EXISTS address text; ALTER TABLE users ADD COLUMN IF NOT EXISTS city text;`);
...
async function api(req,res,path){
  ...
  const publicUser={userId:user.user_id,email:user.email,displayName:user.display_name||'',phone:user.phone||'',address:user.address||'',city:user.city||'',role:user.role,status:user.status};
  if(path==='/api/account/delete'&&req.method==='DELETE'){ if(user.role==='admin')return send(res,403,{error:'The platform administrator account cannot be deleted here.'}); await pool.query('DELETE FROM users WHERE user_id=$1',[user.user_id]); return send(res,200,{ok:true},{'Set-Cookie':setCookie('',0)}); }
